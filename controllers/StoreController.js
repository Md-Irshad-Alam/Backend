...
exports.saveStore = expressAsyncHandler(async (req, res) => {
  try {
    ...
    await StoreModel.create({ store_name, remarks, isActive })
      .then(() => {
        res
          .status(200)
          .json({ message: CommonMessage.storesave.success, success: true });
      })
      .catch((error) => {
        res.status(400).json({
          message: CommonMessage.storesave.failed,
          success: false,
...
exports.getallstore = expressAsyncHandler(async (req, res) => {
  try {
    ...
    let totalPage = Math.ceil((await StoreModel.countDocuments()) / limit);
...
exports.togglestore = expressAsyncHandler(async (req, res) => {
  try {
    ...
    if (!storedetails) {
      res
        .status(200)
        .json({ message: CommonMessage.togglestore.notfound, success: false });
    }
    ...
    await StoreModel.findByIdAndUpdate(
      ...
      .then((result) => {
        res.status(200).json({
          message: result.isActive
            ? CommonMessage.togglestore.active
            : CommonMessage.togglestore.deactive,
          success: true,
          stores: result,
        });
      })
      .catch((error) => {
        res.status(400).json({
          message: CommonMessage.togglestore.failed,
          success: false,
          error: error.toString(),
        });
      });
  ...
  try {
    ...
    await StoreModel.findByIdAndDelete(id)
      .then(() => {
        res
          .status(200)
          .json({ message: CommonMessage.deletestore.success, success: true });
      })
      .catch((error) => {
        res.status(400).json({
          message: CommonMessage.deletestore.failed,
          success: false,
          error: error.toString(),
        });
      });
